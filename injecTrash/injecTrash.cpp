...
int wmain(int argc, wchar_t* argv[])
{
    ...
    wprintf_s(L"DLL Path Location: 0x%08x\n", dllPathAddress);
    ...
    HANDLE hThread;
    NTSTATUS status = NtCreateThreadEx(&hThread, THREAD_ALL_ACCESS, nullptr, hProcess, (LPTHREAD_START_ROUTINE)loadLibraryWAddress, (LPVOID)dllPathAddress, FALSE, 0, 0, 0, nullptr);
    if (status < 0 || !hThread)
    {
        wprintf_s(L"Could not start thread via NtCreateThreadEx (0x%08x)\n", status);
        return 6;
    }
